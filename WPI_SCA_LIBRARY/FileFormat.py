...
def _stored_to_parts(path_value: str) -> list[str]:
    """Convert stored JSON paths from Windows/Linux style into safe path parts."""
    return [p for p in str(path_value).replace("\\", os.sep).replace("/", os.sep).split(os.sep) if p]


def _join_stored(base: str, *stored_paths: str) -> str:
    """Join a base path with one or more stored relative paths in a cross-platform way."""
    parts = []
    for stored_path in stored_paths:
        parts.extend(_stored_to_parts(stored_path))
    return os.path.normpath(os.path.join(base, *parts))


def _metadata_path(base: str) -> str:
    return os.path.join(base, "metadataHolder.json")
...
class FileParent:
    def __init__(self, name: str, path: str, existing: bool = False):
        ...
        if not existing:
            ...
            self.experiments_path = os.path.join(self.path, "Experiments")
            ...
            with open(_metadata_path(self.path), 'w') as json_file:
                json.dump(self.json_data, json_file, indent=4)
            ...
            with open(_metadata_path(path), 'r') as json_file:
                self.json_data = json.load(json_file)
            path_from_json = self.json_data["path"]
            ...
            for experiment in self.json_data["experiments"]:
                if os.path.exists(_join_stored(self.path, experiment["path"])):
                    self.add_experiment_internal(exp_name=experiment.get('name'), existing=True,
                                                 index=experiment.get('index'),
                                                 experiment=experiment)
                else:
    ...
    def add_experiment_internal(self, exp_name: str, existing: bool = False, index: int = 0,
        ...
        if not existing:
            ...
            while not dir_created:
                try:
                    os.mkdir(_join_stored(self.path, exp_path))
                    os.mkdir(os.path.join(_join_stored(self.path, exp_path), "visualization"))
                    dir_created = True
                except FileExistsError:
                    if bool(re.match(r'.*-\d$', exp_name)):
                        ver_num = int(exp_name[len(exp_name) - 1]) + 1
    ...
    def delete_experiment(self, experiment_name: str) -> None:
        ...
        if res == "y" or res == "yes":
            ...
            shutil.rmtree(_join_stored(self.path, self.experiments[experiment_name].path))
            self.experiments.pop(experiment_name)
            for experiment_json in self.json_data["experiments"]:
                if experiment_json["name"] == experiment_name:
                    self.json_data["experiments"].remove(experiment_json)
...
class Experiment:
    def __init__(self, name: str, path: str, file_format_parent: FileParent, existing: bool = False, index: int = 0,
        ...
        if not existing:
            ...
            for dataset in experiment["datasets"]:
                if os.path.exists(_join_stored(self.fileFormatParent.path, self.path, dataset["path"])):
                    self.add_dataset_internal(dataset["name"], existing=True, dataset=dataset)
                else:
                    for experiment_json in self.fileFormatParent.json_data["experiments"]:
                        if experiment_json["name"] == self.name:
                    ...
                    with open(_metadata_path(self.fileFormatParent.path), 'w') as json_file:
                        json.dump(self.fileFormatParent.json_data, json_file, indent=4)
    ...
    def delete_dataset_partition(self, dataset_name: str, partition:bool = False, index:int = None) -> None:
        ...
        if res == "y" or res == "yes":
            print("Deleting dataset {}".format(dataset_name))
            os.remove(_join_stored(self.fileFormatParent.path, self.path, f"{dataset_name}.npy"))
            self.dataset.pop(dataset_name)
    ...
    def get_visualization_path(self) -> str:
        ...
        :rtype: str
        """
        return os.path.join(_join_stored(self.fileFormatParent.path, self.path), "visualization") + os.sep
...
class Dataset:
    ...
    def read_data(self, start: int, end: int) -> np.ndarray:
        ...
        :rtype: np.ndarray
        """
        data = np.load(_join_stored(self.fileFormatParent.path, self.experimentParent.path, self.path))
        return data[start:end]
    ...
    def add_data(self, data_to_add: np.ndarray, datatype: any) -> None:
        ...
        """
        data_to_add = np.array(data_to_add, dtype=datatype)
        np.save(_join_stored(self.fileFormatParent.path, self.experimentParent.path, self.path), data_to_add)
